...
export default function ProjectStart() {
  ...
  return (
    <section id="inicio-proyecto" ref={ref} className="py-16 sm:py-20 md:py-28 relative">
      <div className="section-divider-obsidian mb-16 sm:mb-20" />

      <div className="max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Top Header & Intro */}
        <div className="grid lg:grid-cols-[1fr_1.1fr] gap-8 lg:gap-14 items-start mb-10 sm:mb-12">
          
          <div>
            <div className="reveal flex flex-wrap items-center gap-1.5 sm:gap-2 mb-3 sm:mb-4">
              ...
              <span className="obsidian-tag obsidian-tag-amber text-[10px] sm:text-xs">
                #obligatorio-arranque
              </span>
            </div>

            <h2 className="reveal font-display text-2xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight mb-4 sm:mb-5">
              Primero construimos <span className="gradient-text-purple">la base.</span>
            </h2>

            <p className="reveal text-slate-300 text-sm sm:text-base leading-relaxed mb-5 sm:mb-6 font-normal">
            ...
            <div className="reveal p-4 sm:p-5 rounded-2xl bg-white/[0.03] border border-purple-500/25 shadow-[0_0_30px_rgba(139,92,246,0.1)] mb-5 sm:mb-6">
              <div className="flex flex-wrap items-baseline gap-2 sm:gap-3">
                <span className="font-mono text-2xl sm:text-4xl font-extrabold text-white">
                  {PROJECT_START.price}
                </span>
                <span className="font-mono text-[11px] sm:text-xs font-semibold text-purple-300">
                  + IVA · PAGO ÚNICO
                </span>
              </div>
            ...
            {/* Important Callout */}
            <div className="reveal obsidian-callout obsidian-callout-warning flex items-start gap-2.5 sm:gap-3">
          ...
          <div className="reveal obsidian-card rounded-3xl p-5 sm:p-7 md:p-8 border-purple-500/20">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {PROJECT_START.timeline.map((step) => (
                  <div key={step.step} className="p-2.5 rounded-xl bg-white/[0.02] border border-white/[0.05]">
                    <span className="font-mono text-[9px] sm:text-[10px] font-bold text-purple-400 block mb-1">
                      PASO {step.step}
                    </span>
                    <div className="text-[11px] sm:text-xs font-semibold text-white leading-tight mb-1">{step.title}</div>
                    <div className="text-[9px] sm:text-[10px] text-slate-400 leading-tight">{step.desc}</div>
                  </div>
                ))}
              </div>
            </div>
